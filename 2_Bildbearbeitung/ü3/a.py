...
def scale(img, s_x, s_y):
    ...
    new_img = np.zeros(shape=(new_rows, new_cols, channels))
    for x in range(new_cols):
        for y in range(new_rows):
            position = np.asarray([x, y])
            old_position = np.round(np.matmul(position, T_inv)).astype(int)
            old_x, old_y = old_position[0], old_position[1]
            #überspringen, wenn  außerhalb des Bildes
